...
このツールの版数は `lib/functions.php` の `PV_VERSION` で決めています。**書き換えるのは
ここ1か所**で、画面では一覧の下（フッター）の「⚙ 設定」の右に `v2.0` の形で出ます。

これとは別に、`assets/style.css` と `assets/app.js` の読み込みには `?v=31` のような
番号を付けています。こちらはブラウザに古いファイルを使わせないための番号で、
`PV_VERSION` とは揃えていません。**スタイルを直したら、この番号を増やしてください。**

`style.css` を読んでいるのは次の3つです。1つでも古い番号のまま残ると、その画面だけ
古いスタイルが使われるので、**まとめて同じ番号にします**。

- `index.php`（一覧）
- `lib/init.php`（初期設定の画面）
- `login.php`（ログイン画面）

...
- `/2026/遠足/cover.jpg` … 写真（動画）フォルダの直下からの道順（画面から選ぶとこの形になります）
- `2026/遠足/cover.jpg` … 同じフォルダから探し、無ければ写真（動画）フォルダからの道順として探す
- `random` … そのフォルダ以下の画像から、表示するたびに1枚を選ぶ
- `random:2026/遠足` … 指定したフォルダ以下の画像から、表示するたびに1枚を選ぶ

先頭に `/` を付けた書き方は、写真（動画）フォルダの直下からの道順としてだけ探します。
...
- **画像を選ぶ…** … 写真（動画）フォルダの中をたどって、**どのフォルダの画像でも**選べます
...
選べるのは、**いま開いているタブ（写真・動画）のフォルダの中にある画像**だけです。
動画のフォルダに置いた `info.json` でも、サムネイルには `movies/` の中にある画像を指定します。

...
| 操作 | 動作 |
| --- | --- |
| 「すべて」「写真」「動画」タブ | 一覧に出すものを絞り込む（フォルダは開いたまま） |
...
| 右クリック →「ピン留め」 | フォルダ情報の下に、その写真・動画・フォルダを並べる |
| `←` `→` | 拡大表示中に前後の写真へ（端まで行くと反対側へ回る） |
| `Home` `End` | 最初／最後の写真へ |
| `Esc` | 拡大表示・動画ビューワー・メニュー・モーダル・プロパティを閉じる（何も開いていなければ選択を解除。フォルダ情報の編集画面は閉じません。「画像を選ぶ」の画面は閉じます） |
...
一覧の見せ方・動画の音・動画の大きさは、一覧の下にある「⚙ 設定」から切り替えられます。
...
| 右クリックした場所 | メニューに出る項目 |
| --- | --- |
| 写真・動画 | プロパティ ・ 名前を変更 ・ 移動 ・ ピン留め ・ ダウンロード ・ ゴミ箱へ移動 |
| フォルダ | 名前を変更 ・ 移動 ・ ピン留め ・ ゴミ箱へ移動 |
| 何もない場所 | 新しいフォルダ ・ 複数選択 ・ フォルダ情報 |
| 選択中の写真・フォルダ | 移動 ・ ピン留め ・ ゴミ箱へ移動（選んだもの全部が対象） |

すでにピン留めしてあるものを右クリックすると、「ピン留め」の代わりに「ピン留めを外す」が出ます。
...
写真と動画は同じフォルダに並んでいるので、どちらも同じように移動できます。
絞り込みのタブは移動先には関わりません（「写真」で絞り込んでいても、移動先には
動画の入ったフォルダも選べます）。
...
よく開くフォルダや、代表にしたい写真を**ピン留め**しておくと、
そのフォルダを開いたときに**フォルダ情報の下**に並びます。

- 写真・動画・フォルダを右クリック（スマホは長押し）して「ピン留め」を選びます
- 外すときは、同じように右クリックして「ピン留めを外す」を選びます
- まとめて選んでいるときは、選んだもの全部をまとめて留められます
- 確認の画面は出ません。押し間違えても、もう一度押せば元に戻せます

留めたものは、一覧の中でも 📌 の印が付きます。

ピン留めから押したときの動きは、一覧から押したときと同じです。
写真は拡大表示、動画は再生、フォルダはそのフォルダを開きます。

- 留められるのは、**いま開いているフォルダの中にあるもの**だけです。
  ほかのフォルダのものを留めたいときは、そのフォルダを開いてから操作してください
- 1つのフォルダに留められるのは 30 件までです
- ピン留めは、そのフォルダの `info.json` に `pinned` として書かれます
  （→ [フォルダ情報を付ける](#フォルダ情報を付けるinfojson)）。
  `read_only` が `true` のときや、フォルダに書き込めないときは使えません
- 名前を変えたものはピン留めも付いて変わり、移動・ゴミ箱へ移したものはピン留めから外れます
...
- **受け付ける種類**は、`config.php` の `kinds` に書いた拡張子です。写真も動画もそのまま
  受け取り、それ以外は見送って画面で知らせます。タブで「写真」に絞り込んでいるときでも
  動画を取り込めます（取り込んだあと、「すべて」か「動画」に切り替えると見えます）。
...
- フォルダ情報（`info.json`）の欄と、その下の枚数・ページ送りの上からは始まりません。
  説明の文字をなぞって選ぶときに、枠が出てしまわないようにするためです
...
削除しても、すぐには消えません。`media/.trash/` の下に、実行した日時のフォルダを作り、
**元の場所ごと**移動します。

```
media/.trash/20260823-142530/2026-01_旅行/kyoto-01.jpg
...
```
make ssh
mv ~/www/media-library/media/.trash/20260823-142530/2026-01_旅行/kyoto-01.jpg ~/www/media-library/media/2026-01_旅行/
```
...
整理機能は `media/` に書き込めることが前提です。権限が足りない場合、操作メニューは出ず、
「このフォルダに書き込む権限がないため……」という案内が表示されます。その場合はサーバー上で
`chmod 755 ~/www/media-library/media` などを実行してください。
...
- ピン留めしたものがあれば、そのフォルダ情報の下に並びます（→ [ピン留めする](#ピン留めする)）
- 枚数・ページ番号とページ送りは、さらにその下にそろえて出ます。
...
| `pinned` | 任意 | ピン留め。**そのフォルダ直下の名前**を、並べたい順に書く（30件まで） |

`title` `thumbnail` `items` `pinned` はどれも省略できますが、すべて空のときは
フォルダ情報が無いものとして扱います。

`pinned` に書けるのは、`info.json` と同じフォルダにある写真・動画・フォルダの名前だけです。
`2026/遠足` のようにフォルダをまたぐ書き方や、実体の無い名前は読み飛ばします。
画面からの操作は [ピン留めする](#ピン留めする) を参照してください。

### サムネイルの指定

`thumbnail` には、次の5とおりが書けます。

- `cover.jpg` … `info.json` と同じフォルダにあるファイル名
- `/2026/遠足/cover.jpg` … `media/` の直下からの道順（画面から選ぶとこの形になります）
- `2026/遠足/cover.jpg` … 同じフォルダから探し、無ければ `media/` からの道順として探す
- `random` … そのフォルダ以下の画像から、表示するたびに1枚を選ぶ
- `random:2026/遠足` … 指定したフォルダ以下の画像から、表示するたびに1枚を選ぶ

先頭に `/` を付けた書き方は、`media/` の直下からの道順としてだけ探します。
同じ名前のサブフォルダがあっても、そちらを先に拾ってしまうことがありません。
`/` を付けない書き方では、まず同じフォルダから探し、見つからなければ
`media/` からの道順として探します。
`media/` の外にあるファイルや、外部サイトの画像URLは指定できません
（非公開のギャラリーから外へ通信しないようにするためです）。

指定できるのは画像だけで、種類は `config.php` の `kinds` の `image` に書いた拡張子です。
動画しか入っていないフォルダでも、写真を1枚置けばサムネイルにできます。
...
- **サムネイル** … 「なし」「ランダム」「選んだ画像」から選びます
- **画像を選ぶ…** … `media/` の中をたどって、**どのフォルダの画像でも**選べます
- **ランダムに選ぶ範囲** … 「ランダム」を選んだときだけ出ます。
  「このフォルダ以下」のほか、`media/` の中のどのフォルダでも選べます
- **項目** … 「見出し」「内容」「リンク（任意）」の3つで1行。`↑` `↓` で順番を入れ替え、
  `×` で消し、「項目を追加」で増やせます

#### 画像を選ぶ

「画像を選ぶ…」を押すと、フォルダをたどって画像を選ぶ画面が開きます。

- 開くのは、いま選んである画像のあるフォルダです。
  まだ選んでいなければ、いま開いているフォルダから始まります
- フォルダを押すとその中へ、パス表示（パンくず）や「↑ 上のフォルダへ」を押すと元の階層へ戻ります
- 画像を押すと、その1枚がサムネイルに決まり、この画面は閉じます。
  決めた内容は、下の「保存」を押したときに書き込まれます
- 選ぶのをやめるときは「キャンセル」か `Esc` を押します。フォルダ情報の書きかけはそのまま残ります

選べるのは、**`media/` の中にある画像**だけです。動画しか入っていないフォルダの
`info.json` でも、ほかのフォルダの画像をたどって選べます。
画像が多いフォルダでは、名前順の先頭 300 枚までを並べます。
...
ピン留め（`pinned`）はこの画面には出していないので、書かれていたものをそのまま残します。
**この4つ以外のキーは残りません。**
タイトル・サムネイル・項目をすべて空にして保存すると、フォルダ情報をやめる操作とみなして
`info.json` をゴミ箱へ移動します（`media/.trash/` に残るので、あとから戻せます）。
ピン留めが残っているときは、それだけを書いたファイルとして残ります。
...
- **最後の項目のうしろにカンマを付けないでください。** `"value": "24名",` の `,` などが残っていると
  ファイル全体が読めなくなります
- `title` `thumbnail` `items` `pinned` 以外のキーを書いても、表示には使われません。画面から保存すると消えます
...
- ピン留めしたものは、一覧にも出たまま（同じものが2か所に出る形）です。
  ページ送りや絞り込みで一覧に出ていないときも、ピン留めからは開けます

...
- フォルダに `info.yml` を置くと、そのフォルダ情報を一覧の横に表示（→「フォルダ情報を付ける」）
...
| `Home` `End` | 最初／最後の写真へ |
| `Esc` | 拡大表示・動画ビューワー・メニュー・モーダル・プロパティを閉じる（何も開いていなければ選択を解除） |
...
フォルダ情報（`info.yml`）があるときは、一覧の上ではなく**フォルダ情報の下**に並びます。
...
フォルダの中に `info.yml` という名前のファイルを置くと、そのフォルダの情報として扱われます。
...
```yaml
# このフォルダ以下から選ぶ（うしろに何も付けない書き方）
thumbnail: random

# 「2026/遠足」以下から選ぶ
thumbnail: random:2026/遠足

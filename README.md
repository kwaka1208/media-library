...
- 1行ずつ並べるリスト表示（右上のボタンで切り替え）
  - 項目をクリックすると、右側（画面が狭いときは下）に詳細が出る
  - 詳細にはサムネイル・種類・サイズ・更新日時・場所と、拡大／再生とダウンロードのボタン
...
- フォルダに `info.yml` を置くと、その中身の説明を一覧の横に表示（→「フォルダに説明を付ける」）
...
| 写真をクリック | グリッドでは拡大表示、リストでは詳細を表示 |
| 動画をクリック | グリッドではビューワーで再生、リストでは詳細を表示 |
| リストの項目をダブルクリック | 拡大表示・再生 |
| `←` `→` | 拡大表示中に前後の写真へ（端まで行くと反対側へ回る） |
| `Home` `End` | 最初／最後の写真へ |
| `Esc` | 拡大表示・動画ビューワー・メニュー・モーダル・詳細を閉じる（何も開いていなければ選択を解除） |
...
![リスト表示と詳細](docs/screenshots/list-detail.jpg)
...
リストで項目をクリックすると、右側に**詳細**が出ます。
画面が狭いときは下から出てきて、押した行が隠れない位置まで一覧が動きます。

詳細に出る内容は次のとおりです。

- サムネイル（動画は先頭のコマ。押すと再生が始まります）
- ファイル名
- 種類（写真／動画）・サイズ・更新日時・場所（入っているフォルダ）
- 「拡大表示」（動画は「再生する」）と「ダウンロード」のボタン

クリックしただけでは拡大しないので、行を続けて押しながら見比べられます。
そのまま拡大・再生したいときは、行を**ダブルクリック**してください。

詳細を閉じるときは、右上の「×」か `Esc` キーです。
`Esc` は、拡大表示を開いているときはそちらを先に閉じるので、
詳細は開いたまま残ります。
...
`config.php` の `read_only` を `true` にすると、右クリックしても操作メニューが出なくなり
（ブラウザ本来のメニューが出ます）、画面上部のボタンも消え、`action.php` も処理を受け付けなくなります。
...
## フォルダに説明を付ける（info.yml）

フォルダの中に `info.yml` という名前のファイルを置くと、そのフォルダの説明として扱われます。

- そのフォルダを開いたとき、一覧の左（画面が狭いときは上）に説明が出ます
- 上のフォルダの一覧では、フォルダ名の代わりに `title` が、📁 の代わりにサムネイルが出ます
- 画面上部のパス表示（パンくず）と、ブラウザのタブに出る名前にも `title` が出ます。
  実際のフォルダ名は、パス表示にマウスを乗せると出ます

写真・動画・フォルダを押したときの動きは、説明があってもなくても変わりません。
...
`title` `thumbnail` `items` はどれも省略できますが、すべて空のときは説明が無いものとして扱います。

### サムネイルの指定

`thumbnail` には、次の3とおりが書けます。

- `cover.jpg` … `info.yml` と同じフォルダにあるファイル名
...
動画のフォルダに置いた `info.yml` でも、サムネイルには画像を指定します。
...
「フォルダの説明」を選ぶと、いま開いているフォルダの `info.yml` を編集できます。
...
編集できるのは、いま開いているフォルダの説明だけです。ほかのフォルダの説明を直すときは、
そのフォルダを開いてから操作してください。

保存すると、`title` `thumbnail` `items` を書き出したファイルで置き換わります。
**手で書き足したコメントや、この3つ以外のキーは残りません。**
タイトル・サムネイル・項目をすべて空にして保存すると、説明をやめる操作とみなして
`info.yml` をゴミ箱へ移動します（`photos/.trash/` に残るので、あとから戻せます）。
...
読み取りは `info.yml` に必要な範囲に絞ってあります。

- 対応：`キー: 値` の並び、`- ` で始まるリスト、`#` から行末までのコメント、`"…"` と `'…'` の引用符
- 対応していない：さらに深い入れ子、`|` や `>` で続ける複数行、`&` `*` の参照、`{ }` `[ ]` を使った書き方

値はすべて文字ならびとして読みます。`2026-04-05` や `24` も、書いたとおりの見た目で表示されます。
値の中に `:` を含めたいとき（`10:00 集合` など）は、`"` で囲ってください。

### 注意点

- `info.yml` そのものは、写真・動画の一覧には出ません
...
├── lib/yaml.php        info.yml を読むための、小さなYAMLの読み取り
